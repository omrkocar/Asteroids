...
namespace Saz
{
	...
	ScriptFieldType MonoTypeToScriptFieldType(MonoType* monoType)
	{
		...
		{
			SAZ_CORE_ERROR("Unknown type: {}", typeName);
	...
	void ScriptEngine::Init()
	{
		...
		LoadAppAssembly("Scripts/Binaries/Game.dll");
		LoadAssemblyClasses();

		ScriptGlue::RegisterComponents();
		ScriptGlue::RegisterFunctions();

		s_Data->EntityClass = ScriptClass("Saz", "Entity", true);
	}
	...
	void ScriptEngine::LoadAssemblyClasses()
	{
		...
		int32_t numTypes = mono_table_info_get_rows(typeDefinitionsTable);
		MonoClass* entityClass = mono_class_from_name(s_Data->CoreAssemblyImage, "Saz", "Entity");
		...
		for (int32_t i = 0; i < numTypes; i++)
		{
			...
			SAZ_CORE_WARN("{} fields:", className);
			...
			while (MonoClassField* field = mono_class_get_fields(monoClass, &iterator))
			{
				...
				if (flags & FIELD_ATTRIBUTE_PUBLIC)
				{
					...
					SAZ_CORE_WARN("  {} ({})", fieldName, ScriptFieldTypeToString(fieldType));
	...
	void ScriptEngine::LoadAssembly(const std::filesystem::path& filepath)
	{
		...
		s_Data->AppDomain = mono_domain_create_appdomain("SazScriptRuntime", nullptr);
		...
		: m_ClassNamespace(classNamespace), m_ClassName(className)
	{
		...
		//mono_class_num_fields(m_MonoClass);
