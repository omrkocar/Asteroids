...
RayRectangle texShapesRec = { 0, 0, 1, 1 };        // Texture source rectangle used on shapes drawing
...
void SetShapesTexture(Texture2D texture, RayRectangle source)
{
    texShapes = texture;
    texShapesRec = source;
}
...
{
    DrawRectanglePro((RayRectangle){ position.x, position.y, size.x, size.y }, (Vector2){ 0.0f, 0.0f }, 0.0f, color);
}

// Draw a color-filled rectangle
void DrawRectangleRec(RayRectangle rec, Color color)
{
    DrawRectanglePro(rec, (Vector2){ 0.0f, 0.0f }, 0.0f, color);
}

// Draw a color-filled rectangle with pro parameters
void DrawRectanglePro(RayRectangle rec, Vector2 origin, float rotation, Color color)
{
...
{
    DrawRectangleGradientEx((RayRectangle){ (float)posX, (float)posY, (float)width, (float)height }, color1, color2, color2, color1);
}
...
{
    DrawRectangleGradientEx((RayRectangle){ (float)posX, (float)posY, (float)width, (float)height }, color1, color1, color2, color2);
}

// Draw a gradient-filled rectangle
// NOTE: Colors refer to corners, starting at top-lef corner and counter-clockwise
void DrawRectangleGradientEx(RayRectangle rec, Color col1, Color col2, Color col3, Color col4)
{
    rlSetTexture(texShapes.id);
...
void DrawRectangleLinesEx(RayRectangle rec, float lineThick, Color color)
{
    ...
    RayRectangle top = { rec.x, rec.y, rec.width, lineThick };
    RayRectangle bottom = { rec.x, rec.y - lineThick + rec.height, rec.width, lineThick };
    RayRectangle left = { rec.x, rec.y + lineThick, lineThick, rec.height - lineThick*2.0f };
    RayRectangle right = { rec.x - lineThick + rec.width, rec.y + lineThick, lineThick, rec.height - lineThick*2.0f };
...
void DrawRectangleRounded(RayRectangle rec, float roundness, int segments, Color color)
{
...
void DrawRectangleRoundedLines(RayRectangle rec, float roundness, int segments, float lineThick, Color color)
{
    ...
    if (roundness <= 0.0f)
    {
        DrawRectangleLinesEx((RayRectangle){rec.x-lineThick, rec.y-lineThick, rec.width+2*lineThick, rec.height+2*lineThick}, lineThick, color);
        return;
    }
...
bool CheckCollisionPointRec(Vector2 point, RayRectangle rec)
{
...
bool CheckCollisionRecs(RayRectangle rec1, RayRectangle rec2)
{
    bool collision = false;
...
bool CheckCollisionCircleRec(Vector2 center, float radius, RayRectangle rec)
{
...
RayRectangle GetCollisionRec(RayRectangle rec1, RayRectangle rec2)
{
    RayRectangle rec = { 0, 0, 0, 0 };
