 ...
 * Rectangle type
 */
class Rectangle : public RayRectangle {
 public:
    Rectangle(const RayRectangle& vec) {
        set(vec);
    }

    Rectangle(float x, float y, float width, float height) : RayRectangle{x, y, width, height} {}
    Rectangle(float x, float y, float width) : RayRectangle{x, y, width, 0} {}
    Rectangle(float x, float y) : RayRectangle{x, y, 0, 0} {}
    Rectangle(float x) : RayRectangle{x, 0, 0, 0} {}
    Rectangle() : RayRectangle{0, 0, 0, 0} {}

    Rectangle(::Vector2 position, ::Vector2 size)
            : RayRectangle{position.x, position.y, size.x, size.y} {}
    Rectangle(::Vector2 size) : RayRectangle{0, 0, size.x, size.y} {}
    Rectangle(::Vector4 rect) : RayRectangle{rect.x, rect.y, rect.z, rect.w} {}
    ...
    Rectangle& operator=(const RayRectangle& rect) {
        set(rect);
        return *this;
    }
 ...
 private:
    inline void set(const RayRectangle& rect) {
        x = rect.x;
        y = rect.y;
        width = rect.width;
        height = rect.height;
