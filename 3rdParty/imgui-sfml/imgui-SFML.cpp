...
ImVec2 ToVec(const sf::Vector2i& rhs) { return { static_cast<float>(rhs.x), static_cast<float>(rhs.y) }; }
ImVec2 ToVec(const sf::Vector2f& rhs) { return { rhs.x, rhs.y }; }
ImVec4 ToVec(const sf::Color& rhs) { return { static_cast<float>(rhs.r), static_cast<float>(rhs.g), static_cast<float>(rhs.b), static_cast<float>(rhs.a )}; }
sf::Vector2f ToVec(const ImVec2& rhs) { return { rhs.x, rhs.y }; }
...
namespace ImGui {
	namespace SFML {
		...
		void Init(sf::Window& window, const sf::Vector2f& displaySize, bool loadDefaultFont) {
			...
			// init rendering
			io.DisplaySize = ToVec(displaySize);
	...
	void Image(const sf::Texture& texture, const sf::Vector2f& size,
		...
		ImTextureID textureID =
			convertGLTextureHandleToImTextureID(texture.getNativeHandle());
		ImGui::Image(textureID, ToVec(size), ImVec2(0, 0), ImVec2(1, 1), ToVec(tintColor),
			ToVec(borderColor));
	}
	...
	void Image(const sf::Texture& texture, const sf::Vector2f& size,
		...
		ImTextureID textureID =
			convertGLTextureHandleToImTextureID(texture.getNativeHandle());
		ImGui::Image(textureID, ToVec(size), uv0, uv1, ToVec(tintColor), ToVec(borderColor));
	}
	...
	void DrawLine(const sf::Vector2f& a, const sf::Vector2f& b,
		...
		sf::Vector2f pos = ToVec(ImGui::GetCursorScreenPos());
		draw_list->AddLine(ToVec(a + pos), ToVec(b + pos), ColorConvertFloat4ToU32(ToVec(color)),
			thickness);
	}
	...
	void DrawRect(const sf::FloatRect& rect, const sf::Color& color, float rounding,
		...
		draw_list->AddRect(getTopLeftAbsolute(rect), getDownRightAbsolute(rect),
			ColorConvertFloat4ToU32(ToVec(color)), rounding,
	...
	void DrawRectFilled(const sf::FloatRect& rect, const sf::Color& color,
		...
		draw_list->AddRectFilled(
			...
			ColorConvertFloat4ToU32(ToVec(color)), rounding, rounding_corners);
	}
...
namespace {
	...
	bool imageButtonImpl(const sf::Texture& texture,
		...
		ImTextureID textureID =
			convertGLTextureHandleToImTextureID(texture.getNativeHandle());
		return ImGui::ImageButton(textureID, ToVec(size), uv0, uv1, framePadding, ToVec(bgColor),
			ToVec(tintColor));
	}
