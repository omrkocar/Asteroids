...
bool MyGLContext::AcquireWGLExtensions(HINSTANCE hInstance)
{
    ...
    // Create the window.
    hWnd = CreateWindowA("MyWindowClass",   // Window class.
        "Temp window",                     // Title.
    ...
    {
        MessageBoxA(0, "Release temp device context failed.", "SHUTDOWN ERROR", MB_OK | MB_ICONINFORMATION);
    }

    if (hWnd && !DestroyWindow(hWnd))
    {
        MessageBoxA(0, "Could not release temp hWnd.", "SHUTDOWN ERROR", MB_OK | MB_ICONINFORMATION);
    }
...
{
    Destroy();
    MessageBoxA(0, pMessage, "ERROR", MB_OK | MB_ICONEXCLAMATION);
    return false;
}
...
void MyGLContext::DestroyRenderingContext(HDC hDeviceContext)
{
    if (m_hRenderingContext)
    {
		...
		{
			MessageBoxA(0, "Release of device context and rendering context failed.", "SHUTDOWN ERROR", MB_OK | MB_ICONINFORMATION);
		}

		if (!wglDeleteContext(m_hRenderingContext))
		{
			MessageBoxA(0, "Release rendering context failed.", "SHUTDOWN ERROR", MB_OK | MB_ICONINFORMATION);
		}
    }
